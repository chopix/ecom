...
@section('content')
  ...
  <form action="{{route('admin.tools.active.update', $tool->id)}}" method="POST">
    @method('PUT')
    @csrf
    @if ($tool->is_active)
      <button type="submit" class="btn btn-danger" name="is_active" value="false">Deactivate this tool</button>
    @else
      <button type="submit" class="btn btn-primary" name="is_active" value="true">Activate this tool</button>
    @endif
  </form>
